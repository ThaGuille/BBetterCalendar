...
- **Slug**: short, descriptive, kebab-case. Reuse an existing slug if updating that plan;
  otherwise pick a new descriptive one (`add-reminder-popup`, not an auto-generated name).
...
2. Use today's date from the environment (the `CLAUDE.md` `currentDate`, or the system clock).
3. Write the file with the **Write** tool. Don't `git add`/`commit` unless the user asks —
   these files are intentionally uncommitted working history.
4. Link from related code only if useful via a `// see .claude/plans/<slug>.md` comment.

## Don't

- Don't delete or overwrite an existing plan with a different topic — pick a new slug.
