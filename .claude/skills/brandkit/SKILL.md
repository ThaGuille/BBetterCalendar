---
name: brandkit
description: GENERATES IMAGES of brand identity material — logo concepts and lockups, brand-guideline boards, colour/type specimen sheets, app-icon directions, identity decks and visual-world moodboards. Use ONLY for brand/identity artefacts, e.g. "design a logo for BBetter", "make a brand board", "app icon concepts", "moodboard for the new direction". Outputs images ONLY; writes no code. For app SCREEN mockups use `mobile-mockup`; for in-app visual design use `impeccable`.
---
