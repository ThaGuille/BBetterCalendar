---
name: impeccable
description: PRIMARY DESIGN SKILL and the design authority for this repo — the DEFAULT for any request to design, redesign, shape, critique, audit, polish, clarify, distill, harden, optimize, adapt, animate, colorize, delight, or otherwise improve the app's UI. Covers Android app screens, app shells, components, forms, settings, onboarding and empty states. Handles UX review, visual hierarchy, information architecture, cognitive load, accessibility, theming, anti-patterns, typography, fonts, spacing, layout, alignment, colour, motion, micro-interactions, UX copy, error states, edge cases, i18n, and reusable design systems or tokens. Also for bland designs that should become bolder or more delightful, loud designs that should become quieter, and ambitious visual effects. It DECIDES and EDITS; it calls `ui-ux-pro-max` for concrete values and `material-3` for MD3 spec. Not for backend-only or non-UI tasks.
version: 4.3.1
user-invocable: true
...
>
> **This skill is the design authority for this repo.** When any other design skill or
> plugin (including the user-scope `frontend-design` plugin) disagrees about aesthetic
> direction, critique, or quality floor, this one wins.
>
> **Platform.** Java + **Android XML Views**, Material Components 1.9.0, minSdk 21 /
> target 34. Not Compose, not web. Load [`reference/android.md`](reference/android.md),
> [`reference/adapt.native.md`](reference/adapt.native.md) and
> [`reference/audit.native.md`](reference/audit.native.md) — **never** the web/browser
> paths. The `live` command and every `live-browser*.js` script are inapplicable: there is
> no browser. Deliverables are `res/layout/*.xml`, `res/values/*.xml`, `res/drawable/*.xml`,
> `res/anim/*.xml` and the Java that drives them.
>
> **Mode is `Operate`.** BBetter is task-completion product UI (timer, habits, calendar,
> projects) — scanability, consistency and native expectations outrank expression. Brand
> lives in precise details, not hero moments.
>
> **Project rules override the craft floor where they conflict** — see
> [`CLAUDE.md`](../../../CLAUDE.md). Non-negotiable: colours go through `bb_*` semantic
> tokens and type through `TextAppearance.BBetter.*` (rule #2); never a raw hex in a
> layout. Material 1.9.0 is pinned — do not propose components that need a newer version.
>
> **Verification is delegated, not run here.** Do NOT invoke `adb` yourself. Project rule
> #7 routes emulator verification through the **`ui-tester` subagent**, which drives the
> `adb-ui-test` skill in an isolated context so uiautomator dumps and logcat stay out of
> this conversation. Ask for it; read its pass/fail summary. Dark theme and font-scale
> 1.3 belong in that pass.
>
> **Existing project context beats `init`/`document`.** `PRODUCT.md` and `DESIGN.md` do
> not exist here by those names — the equivalents are [`CLAUDE.md`](../../../CLAUDE.md),
> [`.claude/docs/style_guide.md`](../../docs/style_guide.md) and
> [`.claude/docs/systems/`](../../docs/systems/). Read those instead of inventing context,
> and write changes back to them rather than creating parallel files.
>
> **The engine binary is not installed.** `scripts/impeccable` / `impeccable.cmd` would
> download it on first run, and the author notes the Windows launcher is untested on real
> Windows. Skip the `scripts/impeccable context` setup step and follow the SKILL's own
> documented fallback: say context loading did not run, read project context directly,
> and continue. The `reference/*.md` playbooks are the substance and work without it.
>
> **Reach for siblings instead of guessing:** `ui-ux-pro-max` for concrete palette / font /
> chart / icon values, `material-3` for what MD3 specifies, `image-to-code` when the user
> supplies a reference screenshot, `mobile-mockup` to show image options before building.
