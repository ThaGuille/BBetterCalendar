...
name: material-3
description: >
  SPEC AUTHORITY for Google's Material Design 3 (Material You). Use when the question is
  "what does MD3 actually say" about: colour roles (primary/on-primary/surface-variant/
  outline/error), the type scale (Display/Headline/Title/Body/Label x large/medium/small),
  shape scale, tonal elevation levels, motion tokens (container transform, shared-axis,
  fade-through, easing + durations), component anatomy and states, adaptive/responsive
  breakpoints, dynamic colour, and MD3 accessibility minimums. Use when: "material design",
  "MD3", "material you", "material component", "elevation", "type scale", "colour role".
  NOTE: this project is Java + Android XML Views on Material Components 1.9.0 — read the
  spec here and translate to XML/themes; the skill's code samples are Compose and must not
  be pasted in. Does NOT decide aesthetic direction or run critiques — `impeccable` owns that.
user-invokable: true
