---
name: mobile-mockup
description: GENERATES IMAGES of Android app screen concepts and multi-screen flows — visual mockups to look at and choose from, before any XML is written. Use when the user wants to SEE options ("show me what X could look like", "mock up the progress screen", "give me 3 directions for the timer"). Prioritizes clean hierarchy, readable text, multi-screen consistency, controlled palettes, non-generic art direction, textured surfaces, and custom iconography. Frame screens in an ANDROID device mockup (this is an Android app — never an iPhone frame). This skill outputs images ONLY; it writes no code. For identity/logo boards use `brandkit`; to turn a chosen mockup into real XML use `image-to-code`.
---

> ## ⚠️ PROJECT OVERRIDE — BBetterCalendar (read first, outranks everything below)
>
> This project ships **Android only**. Wherever the guidance below defaults to iOS, an
> iPhone frame, or "iOS-native premium", substitute the Android equivalent:
>
> - **Device frame:** an Android phone (Pixel-class), never an iPhone.
> - **Patterns:** Material Design 3 — navigation bar (3–5 destinations), top app bar,
>   FAB for the single primary action, snackbars for transient feedback, bottom sheets.
>   Never Cupertino switches, iOS dialogs, or a back-chevron that ignores system Back.
> - **Type:** the app's own faces (Plus Jakarta Sans body / Fraunces display) unless the
>   brief is explicitly exploring a new direction.
> - **Palette:** the `bb_*` semantic tokens, unless the brief is exploring a new direction.
>   See [`.claude/docs/style_guide.md`](../../docs/style_guide.md).
> - **Screens to mock:** Home (Pomodoro), Progress (charts + usage bands), Calendar
>   (month/week), Projects.
>
> Output is **images only** — no XML, no code. To implement a chosen direction, hand the
> image to `image-to-code`.
