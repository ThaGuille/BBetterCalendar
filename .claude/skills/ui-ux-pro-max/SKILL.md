---
name: ui-ux-pro-max
description: "LOOKUP LIBRARY for concrete design values — use when a specific VALUE is needed, not a decision: a colour palette for a product category, a font pairing, a named style recipe (glassmorphism, Swiss, brutalist...), a chart type for a data shape, an icon name, or the text of a UX guideline. Query it offline via scripts/search.py. Local data: 79 styles (50 active), 192 product palettes with contrast-corrected pairs, 74 font pairings, 119 UX guidelines, 105 icons, 25 chart types, 17 GSAP presets, 22 stacks. NOT the skill that decides aesthetic direction, critiques, audits or edits UI — `impeccable` owns that and calls this one for raw values. NOT the authority on Material Design 3 — `material-3` owns that."
---
