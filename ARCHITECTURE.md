...
BBetterCalendar is a single-module Android app written in **Java 8** that combines a Pomodoro-style study timer with a calendar for managing events, tasks, and reminders. It tracks daily study time, streaks, and failed timer sessions.
...
> **Do not upgrade** `appcompat` past 1.6.1 or `Room` past 2.4.2 -- see warning comments in `app/build.gradle`.
...
| Class | Role |
|-------|------|
| `CalendarEntry` | Room `@Entity`. Fields: title, description, dates (`Calendar`), notifications (`boolean[7]`), repetition, duration, isDone, type. |
| `CalendarEntry.EventBuilder` | Serializable builder (pattern). Separate setters for day vs hour components of start/end. |
...
| `InitialConfiguration` | Legacy. Extends `AppCompatActivity` but used as a singleton -- its logic has been migrated to `SplashActivity`. The `initialize()` call in `MainActivity` is commented out. |

### `database`

| Class | Role |
|-------|------|
| `AppDatabase` | Room `@Database` (version 6). Singleton via double-checked locking. Currently uses `fallbackToDestructiveMigration()` -- schema changes wipe all data. Exposes `eventDao()`, `statsDao()`, `configurationDao()`. |
...
| Class | Role |
|-------|------|
| `FormatHelper` | Static methods: `formatTime(millis, mask)` (supports "HH:mm" and "mm:ss"), date/time string formatting, millis<->minutes conversion. |
| `ScreenHelper` | Static methods: screen dimensions, navigation/status bar heights, dp<->px conversion. |
...
| `OnToolBarListener` | Interface: `onToolbarLoaded(int result)`. |
| `OnToolbarCalendarListener` | Interface: `switchFragment()`. |
| `OnToolbarHomeListener` | Interface: `onToolbarTimerClick()`. |
...
| `OnNotificationsPopupListener` | Interface: `OnSetNotifications(boolean[])`. |
...
| `RepetitionPopup` | 4 toggle buttons (none, daily, weekly, monthly). Auto-dismisses after 750ms. Returns selected int constant. |
...
| `StatsDAO` | Room DAO with fine-grained update queries (`addTimeStudied`, `addFails`, `addTasksDone`, `resetDailyStats`, streak management). |
...
| Class | Role |
|-------|------|
| `HomeFragment` | Timer state machine with 6 states (STOPPED / RUNNING / PAUSED x normal / rest). Uses `CountDownTimer`. Detects background via `ActivityLifecycleCallbacks` -- fails the timer after 4 seconds in background. Manages concentration/rest cycles. |
...
| Class | Role |
|-------|------|
| `CalendarFragmentMonth` | Month grid view. Draws horizontal/vertical grid lines programmatically. FAB to add events. Switches to week view via Navigation action. |
| `CalendarFragmentWeek` | Week view with hour labels. Uses `RecyclerView` + `CalendarWeekAdapter`. Programmatic grid lines. |
| `CalendarViewModel` | Minimal -- holds a single `MutableLiveData<String>`. Intended for storing calendar state across config changes. |
| `CalendarController` | Shared logic for month/week fragments: handles `ActivityResult` callbacks, formats and logs events, positions week-day initials text with `SpannableStringBuilder`. |
| `CalendarWeekAdapter` | `RecyclerView.Adapter` for week view event items. Currently shows hardcoded "EUREKA" text. |

### `ui.progress` / `ui.projects`

Placeholder screens with stub ViewModels that display static text. No real functionality yet.
...
Background detection:
  ActivityLifecycleCallbacks.onActivityStopped -> if TIMER_RUNNING, wait 4s -> failTimer()
...
```
CalendarFragmentMonth (or Week) -> FAB click
...
Back in calendar fragment:
  -> CalendarController.handleOnActivityResult() -> logs all events (debug)
```
...
Start destination: `HomeFragment`. The calendar month and week fragments are connected by explicit navigation actions and share a toolbar button to switch between them.

---

## Known quirks and tech debt

- **DBMigration naming**: `DBMigration` is the `Application` class (registered in manifest as `android:name`) *and* holds Room migration objects. The name suggests it only does migrations.
- **InitialConfiguration is legacy**: Extends `AppCompatActivity` but is instantiated as a singleton. Its `initialize()` call in `MainActivity` is commented out; all its logic now lives in `SplashActivity`.
...
- **Destructive migration**: `fallbackToDestructiveMigration()` is active -- any Room schema version bump wipes the database. The commented-out migration-based approach is in `AppDatabase`.
- **TODO comments**: Numerous across the codebase, especially around notification layout management, auto-cycle behavior, and the event/task layout switching mechanism in `AddEventActivity`.
- **Stub screens**: Progress and Projects fragments show static placeholder text.
- **Mixed-language comments**: Spanish, Catalan, and English comments coexist throughout.
- **CalendarWeekAdapter**: Hardcodes "EUREKA" text instead of displaying actual event data.
- **Thread safety**: `ConfigurationManager.getConfiguration()` can return null if the background load hasn't completed yet.
