...
| **`minifyEnabled true` + ProGuard/R8 rules** for release builds | Phase 4b (2026-07-05) | Left `false` for the first submission — untested ProGuard rules against Hilt/Room/Gson reflection is not a fight to pick during a first release. APK ships larger but correct. | When APK size matters, or before a release where we've written and verified keep-rules for the reflection-based libs. |
| **F-Droid distribution channel** | Phase 4b (2026-07-05) | First release targets Play only; F-Droid needs a reproducible-build metadata + its own submission pipeline. Out of scope for getting to a first store presence. | If we want an open-source-friendly channel alongside Play, or Play review blocks the accessibility declaration. |
| **Custom domain for the privacy policy** | Phase 4b (2026-07-05) | The free `thaguille.github.io/BBetterCalendar/...` GitHub Pages URL satisfies Play's public-URL requirement; a custom domain is cosmetic. | If the app gets its own domain/branding and we want the policy under it. |
| **Store-listing graphic assets** (icon rework, feature graphic, phone screenshots) | Phase 4b (2026-07-05) | The [release guide](../docs/legal/play-release-guide.md) lists the required dimensions (512px icon, feature graphic, screenshots) but creating the artwork is a separate design ask, not engineering. | When preparing the actual store listing — needs a design pass, not a code change. |
